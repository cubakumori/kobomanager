...
// Solo para admins: columnas/tablas que el código espera y faltan en la BD (deploy
// sin migrar). El front muestra un aviso accionable en vez de esperar a un 500 opaco.
if (($user['role'] ?? '') === 'admin') {
    $user['schema_missing'] = array_map(
        fn($m) => $m['column'] === null ? $m['table'] . ' (tabla)' : $m['table'] . '.' . $m['column'],
        SchemaCheck::missing()
    );
}
