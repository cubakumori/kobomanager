...
const { t } = useI18n()
const { effective, showToggle, cycle } = useDarkMode()

const label = computed(() => t('common.theme') + ': ' + t('common.theme_' + effective.value))
...
<template>
  <button v-if="showToggle" type="button" :class="btnClass" :title="label" :aria-label="label" @click="cycle">
    <!-- Sol (claro) -->
    <svg v-if="effective === 'light'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="h-5 w-5">
    ...
    <!-- Luna (oscuro) -->
    <svg v-else-if="effective === 'dark'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="h-5 w-5">
